...
char Serial_RxPacket_K[100]; // 定义接收数据包数组
int k230_w;                  // K230检测到的框宽
int k230_h;                  // K230检测到的框高
int k230_flag;               // K230检测标志位 0=检测到 1=未检测到
uint8_t Serial_RxFlag_K;     // 定义接收数据包标志位
...
void Serial_SendArray_K(uint8_t *Array, uint16_t Length)
{
	...
	{
		Serial_SendByte_K(Array[i]); // 依次调用Serial_SendByte发送每个字节数据
	}
}
...
void Serial_SendString(char *String)
{
	...
	{
		Serial_SendByte_K(String[i]); // 依次调用Serial_SendByte发送每个字节数据
	}
}
...
void Serial_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		Serial_SendByte_K(Number / Serial_Pow(10, Length - i - 1) % 10 + '0'); // 依次调用Serial_SendByte发送每位数字
	}
}
...
{
	Serial_SendByte_K(ch); // 将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
{
	sscanf(Serial_RxPacket_K, "%d,%d,%d", &k230_w, &k230_h, &k230_flag);
}
...
void USART2_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET) // 判断是否是USART2的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '$' && Serial_RxFlag_K == 0) // 如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;   // 置下一个状态
		...
		else if (RxState == 1)
		{
			...
			{
				Serial_RxPacket_K[pRxPacket] = RxData; // 将数据存入数据包数组的指定位置
				pRxPacket++;						   // 数据包的位置自增
			}
		}
		/*当前状态为2，接收数据包第二个包尾*/
		else if (RxState == 2)
		{
			...
			{
				RxState = 0;						 // 状态归0
				Serial_RxPacket_K[pRxPacket] = '\0'; // 将收到的字符数据包添加一个字符串结束标志
				Serial_RxFlag_K = 1;				 // 接收数据包标志位置1，成功接收一个数据包
			}
		}
